 ...
 */
 ...
 /*Edits by Ethan:
   Any edits are noted with the original being in a comment
   */
...
static inline int __always_unused imx385_read_reg(struct imx385 *imx385, u16 addr, u8 *value)
{
	unsigned int regval;
	int ret;
			  ...
			  struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY)
		framefmt = v4l2_subdev_get_try_format(&imx385->sd, sd_state,
						      fmt->pad);
	else
				  ...
				  struct v4l2_subdev_state *sd_state)
{
	...
	imx385_set_fmt(subdev, sd_state, &fmt);
...
static int imx385_set_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable) {
		ret = pm_runtime_resume_and_get(imx385->dev);
		if (ret < 0)
